...
def train_test_ML(path, method, scoring, season, best_parameters):
    metrics = {}
    ...
    et = time.time()
    if method == 'SVR':
        model, best_parameters, train_predicted_Y, error_train, error_validation = lib.train_SVR(X_train,Y_train, scoring)
    elif method == 'RF':
        model, best_parameters, train_predicted_Y, error_train, error_validation = lib.train_RF(X_train,Y_train, scoring)
    st = time.time()
    ...
    lib.BC_plot(unscaled_test_Y, unscaled_test_predicted_Y, datetime_df, scaler, season)
